...
GLUSvoid mouse(GLUSboolean pressed, GLUSuint button, GLUSuint x, GLUSuint y)
{
  ...
  {
    input.ldown=true;		//установить флаг
    input.mx=x;			//Запомнить координаты
    input.my=y;
  }
  
  if (button & 4)	//Правая кнопка
  {
    input.rdown=true;
    input.mx=x;
    input.my=y;
...
GLUSvoid mouseMove(GLUSuint button, GLUSint x, GLUSint y)
{
  if(button & 1)		// Левая кнопка
  {
    ...
    input.cam_rot[0] += 0.25f*(y1-input.my);	//Изменение угола поворота
...
GLUSboolean update(GLUSfloat time)
{
  try 
  {
    ...
    //
    glUseProgram(g_program.program);  OGL_CHECK_FOR_ERRORS;  
